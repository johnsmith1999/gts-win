...
static void
g_list_validate_allocator (GAllocator *gallocator)
{
  g_return_if_fail (gallocator != NULL);
  g_return_if_fail (gallocator->is_unused == TRUE);

  if (gallocator->type != G_ALLOCATOR_LIST)
    {
      gallocator->type = G_ALLOCATOR_LIST;
  ...
  if (!gallocator->mem_chunk)
    {
      gallocator->mem_chunk = g_mem_chunk_new (gallocator->name,
					      sizeof (GList),
					      sizeof (GList) * gallocator->n_preallocs,
					      G_ALLOC_ONLY);
...
void
g_list_push_allocator(GAllocator *gallocator)
{
  G_LOCK (current_allocator);
  g_list_validate_allocator ( gallocator );
...
g_list_alloc (void)
{
  ...
  if (!current_allocator)
    {
      GAllocator *gallocator = g_allocator_new ("GLib default GList allocator",
					       128);
      g_list_validate_allocator (gallocator);
      gallocator->last = NULL;
      current_allocator = gallocator;
