		 ...
		 guint        n_preallocs)
{
  ...
  gallocator = g_new0 (GAllocator, 1);
  gallocator->name = g_strdup (name);
  gallocator->n_preallocs = CLAMP (n_preallocs, 1, 65535);
  gallocator->is_unused = TRUE;
  gallocator->type = 0;
  gallocator->last = NULL;
  gallocator->mem_chunk = NULL;
  gallocator->dummy = NULL;

  return gallocator;
}

void
g_allocator_free (GAllocator *gallocator)
{
  g_return_if_fail (gallocator != NULL);
  g_return_if_fail (gallocator->is_unused == TRUE);

  g_free (gallocator->name);
