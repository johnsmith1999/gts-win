...
static void
g_node_validate_allocator (GAllocator *gallocator)
{
  g_return_if_fail (gallocator != NULL);
  g_return_if_fail (gallocator->is_unused == TRUE);

  if (gallocator->type != G_ALLOCATOR_NODE)
    {
      gallocator->type = G_ALLOCATOR_NODE;
  ...
  if (!gallocator->mem_chunk)
    {
      gallocator->mem_chunk = g_mem_chunk_new (gallocator->name,
					      sizeof (GNode),
					      sizeof (GNode) * gallocator->n_preallocs,
					      G_ALLOC_ONLY);
      gallocator->free_nodes = NULL;
    }
...
void
g_node_push_allocator (GAllocator *gallocator)
{
  G_LOCK (current_allocator);
  g_node_validate_allocator ( gallocator );
...
g_node_new (gpointer data)
{
  ...
  if (!current_allocator)
    {
       GAllocator *gallocator = g_allocator_new ("GLib default GNode allocator",
						128);
       g_node_validate_allocator (gallocator);
       gallocator->last = NULL;
       current_allocator = gallocator;
