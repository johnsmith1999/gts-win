...
static GtsFace * closest_face (GtsSurface * s, GtsPoint * p)
{
  ...
  for (i = 0, n = 0; i < k && n < ns; i++)
    for (node = hash_table->nodes[i]; node && n < ns; node = node->next) {
      GtsFace * f = node->key;

      if (gts_triangle_orientation (GTS_TRIANGLE (f)) > 0.) {
	GtsPoint * p1 = GTS_POINT (GTS_SEGMENT (GTS_TRIANGLE (f)->e1)->v1);
	d = (p->x - p1->x)*(p->x - p1->x) + (p->y - p1->y)*(p->y - p1->y);
			       ...
			       GtsSurface * surface)
{
  ...
  if (!prev) {
	...
	*/
    /* s intersects f exactly on a summit: restarts from a correct neighbor of f */
    gts_triangle_vertices (GTS_TRIANGLE (f), &v1, &v2, &v3);
    if (gts_point_orientation (GTS_POINT (v2), GTS_POINT (v1), p) > 0.0)
		f1 = neighbor (f, GTS_TRIANGLE (f)->e1, surface);
    if (!f1  && gts_point_orientation (GTS_POINT (v3), GTS_POINT (v2), p) >0.0)
		f1 = neighbor (f, GTS_TRIANGLE (f)->e2, surface);
    if (!f1 && gts_point_orientation (GTS_POINT (v1), GTS_POINT (v3), p) >0.0)
		f1 = neighbor (f, GTS_TRIANGLE (f)->e2, surface);
...
//    g_assert (o2 == 0.);
  ...
  //  NEXT_CUT (e1, e2, left)
    ...
    gdouble o3 = gts_point_orientation (GTS_POINT (s->v1), GTS_POINT (s->v2),
					GTS_POINT (v3));
    //if (o3 > 0.)
    g_assert (o3 != 0.); /* case o3==0 not implemented yet! ref = ???*/
    if (o3 == 0.) {
      remove_triangles (e, surface);
      if (!constraint && !e->triangles)
        gts_object_destroy (GTS_OBJECT (e));
      ...
      *right = g_slist_prepend (*right, e1);
      //GtsFace * ref ;
      return g_slist_concat (constraint,
             remove_intersected_vertex (s, v3, surface, left, right, &ref));
    }
	//
    else if (o3 > 0.)
      NEXT_CUT (e1, e2, left)
			   ...
			   GtsFace ** ref)
{
  ...
  while (i) {
	...
	*right = g_slist_prepend (*right, e1);
	...
	///add
        if (o1 == 0.) {
          *left = g_slist_prepend (*left, e);
          return remove_intersected_vertex (s, v2, surface, left, right, ref);
        }
        if (o2 == 0.) {
          *right = g_slist_prepend (*right, e);
          return remove_intersected_vertex (s, v3, surface, left, right, ref);
        }
	g_assert (next);
