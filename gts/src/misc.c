...
gint gts_file_getc (GtsFile * f)
{
  ...
  while (char_in_string ((char)c, f->comments)) {
    while (c != EOF && c != '\n')
      c = next_char (f);
    if (c == '\n') {
      f->curline++;
...
void gts_file_next_token (GtsFile * f)
{
  ...
  f->token->len = 0;
  if (f->next_token != '\0') {
    if (char_in_string ((char)f->next_token, f->tokens)) {
      f->line = f->curline;
      f->pos = f->curpos - 1;
      g_string_append_c (f->token,(char) f->next_token);
      f->type = f->next_token;
      f->next_token = '\0';
      return;
    }
  ...
  f->type = GTS_NONE;
  while (c != EOF && (!in_string || !char_in_string ((char)c, f->delimiters))) {
    if (in_string) {
      if (char_in_string ((char)c, f->tokens)) {
	f->next_token = c;
	break;
      }
      g_string_append_c (f->token, (char)c);
    }
    else if (!char_in_string ((char)c, f->delimiters)) {
      in_string = TRUE;
      f->line = f->curline;
      f->pos = f->curpos - 1;
      g_string_append_c (f->token, (char)c);
	...
	if (f->type != '=')
	  ...
	  case GTS_FLOAT:
	    ...
	    if (f->type != GTS_INT && f->type != GTS_FLOAT) {
	      ...
	      *((gfloat *) var->data) = (gfloat)atof (f->token->str); 
	    break;
	  case GTS_DOUBLE:
	    gts_file_next_token (f);
	    if (f->type != GTS_INT && f->type != GTS_FLOAT) {
