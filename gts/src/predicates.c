...
*/
  ...
  if ((detleft >= 0.0 && detright <= 0.0) ||
      (detleft <= 0.0 && detright >= 0.0)) {
       FPU_RESTORE;
       return det;
     }
  detsum = Absolute (detleft + detright);
  errbound = ccwerrboundA * detsum;
  if ((det >= errbound) || (-det >= errbound)) {
       FPU_RESTORE;
       return det;
     }
  detleft = (pb[0] - pa[0]) * (pc[1] - pa[1]);
  detright = (pb[1] - pa[1]) * (pc[0] - pa[0]);
  det = detleft - detright;
  ...
  detleft = (pc[0] - pb[0]) * (pa[1] - pb[1]);
  detright = (pc[1] - pb[1]) * (pa[0] - pb[0]);
  det = detleft - detright;
