...
static void write_vertex_msh (GtsPoint * p, gpointer * data)
{
  FILE * fp = data[0];

  fprintf (fp, "%u %.12g %.12g %.12g\n", *((guint *)data[1]), p->x, p->y, p->z);
  GTS_OBJECT (p)->reserved = GUINT_TO_POINTER ((*((guint *) data[1]))++);
}

static void write_face_msh (GtsTriangle * t, gpointer * data)
{
  FILE * fp = data[0];

  GtsVertex * v1, * v2, * v3;
  gts_triangle_vertices (t, &v1, &v2, &v3);
  fprintf (fp, "%u 2 3 %u %u 0 %u %u %u\n",
           (*((guint *)data[1]))++,
           *((guint *)data[2]),
           *((guint *)data[2]),
           GPOINTER_TO_UINT (GTS_OBJECT (v1)->reserved),
...
/**
+ * gts_surface_write_msh:
 * @s: a #GtsSurface.
 * @fptr: a file pointer.
 *
 * Writes in the file @fptr a msh representation of @s.
 */
void gts_surface_write_msh (GtsSurface * s, FILE * fptr, int nsup)
{
  guint n = 1;
  gpointer data[3];
  GtsSurfaceStats stats;
  ...
  fprintf (fptr,
           ...
           "%u\n",
           stats.edges_per_vertex.n);
  gts_surface_foreach_vertex (s, (GtsFunc) write_vertex_msh, data);
  fprintf (fptr,
           "$EndNodes\n"
           "$Elements\n"
           "%u\n",
           stats.n_faces);
  n = 1;
  gts_surface_foreach_face (s, (GtsFunc) write_face_msh, data);
...
static void add_to_list (gpointer item, GSList ** data)
{
  *data = g_slist_append (*data, item);
}

/**
 * gts_surface_vertices:
 * @s: a #GtsSurface.
 *
 * Returns: a list with the vertices of @s.
 */
GSList * gts_surface_vertices (GtsSurface * s)
{
  GSList * vertices = NULL;

  gts_surface_foreach_vertex (s, (GtsFunc) add_to_list, &vertices);
  return vertices;
}
 /**
 * gts_surface_edges:
 * @s: a #GtsSurface.
 *
 * Returns: a list with the edges of @s.
 */
GSList * gts_surface_edges (GtsSurface * s)
{
  GSList * edges = NULL;

  gts_surface_foreach_edge (s, (GtsFunc) add_to_list, &edges);
  return edges;
}
 ...
 * @s: a #GtsSurface.
 *
 * Returns: a list with the faces of @s.
 */
GSList * gts_surface_faces (GtsSurface * s)
{
  GSList * faces = NULL;

  gts_surface_foreach_face (s, (GtsFunc) add_to_list, &faces);
  return faces;
}
...
static void gts_face_orientate (GtsFace * f)
{
	GSList * triangles;
	GTS_OBJECT (f)->reserved = GUINT_TO_POINTER (1);
	
	triangles = (GSList * ) GTS_TRIANGLE (f)->e1->triangles;
  while (triangles) {
    if (GTS_OBJECT(triangles->data)->reserved == NULL) {
      if (!gts_triangles_are_compatible (triangles->data, GTS_TRIANGLE(f), GTS_TRIANGLE (f)->e1))
        gts_triangle_revert (triangles->data);

      gts_face_orientate (triangles->data);
    }
    triangles = triangles->next;
  }

  triangles = GTS_TRIANGLE (f)->e2->triangles;
  while (triangles) {
    if (GTS_OBJECT(triangles->data)->reserved == NULL) {
      if (!gts_triangles_are_compatible (triangles->data, GTS_TRIANGLE(f), GTS_TRIANGLE (f)->e2))
        gts_triangle_revert (triangles->data);
  ...
  triangles = GTS_TRIANGLE (f)->e3->triangles;
  while (triangles) {
    if (GTS_OBJECT(triangles->data)->reserved == NULL) {
      if (!gts_triangles_are_compatible (triangles->data, GTS_TRIANGLE(f), GTS_TRIANGLE (f)->e3))
        gts_triangle_revert (triangles->data);
...
/**
 * gts_surface_orientate:
 * @s: a #GtsSurface.
 *
 * Orient a surface.
 *
 */
void gts_surface_orientate (GtsSurface * s)
{
	GSList * si;
	GSList * surfaces;
  /* check surface */
  g_assert (gts_surface_is_manifold (s));

  /* se marcan las superficies */
  gts_surface_foreach_face (s, (GtsFunc) gts_object_reset_reserved, NULL);

  /* se itera sobre cada sub superficie */
  surfaces = (GSList * )gts_surface_split (s);
  si = (GSList * )surfaces;
  while (si) {
    GSList * si_faces = gts_surface_faces (si->data);
    gts_face_orientate (si_faces->data);
    if (gts_surface_volume (si->data) < 0)
     gts_surface_foreach_face (si->data, (GtsFunc)gts_triangle_revert, NULL);

    gts_object_destroy (GTS_OBJECT (si->data));
    g_slist_free (si_faces);
    si = si->next;
  }
  gts_surface_foreach_face (s, (GtsFunc) gts_object_reset_reserved, NULL);
  g_slist_free (surfaces);
}
